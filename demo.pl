   ...
   if ( $error )
   {
	...
	elsif ( $props{type} eq "suburban" )
	{
	 	$line = sprintf("%-11.11s %-20.20s %-20.20s %-3.3s %4d %-10.10s\n",
	      ...
	      $comps{state},$comps{post_code},$comps{country});
		print(OUT_FH $line);
	}
   elsif ( $props{type} eq "rural" )
   {
	 	$line = sprintf("%-11.11s %-20.20s %-20.20s %-3.3s %4d %-10.10s\n",
	   	"",$comps{property_name},$comps{suburb},$comps{state},$comps{post_code},$comps{country});
		print(OUT_FH $line);
   }
   elsif ( $props{type} eq "post_box" )
   {
	 	$line = sprintf("%-11.11s %-20.20s %-20.20s %-3.3s %4d %-10.10s\n",
	   	"",$comps{post_box},$comps{suburb},$comps{state},$comps{post_code},$comps{country});
		print(OUT_FH $line);
   }
}
close(ERROR_FH);
...
Level 2/12 LEIGH CIRCUIT WERRIBEE HILLS VIC 3030
